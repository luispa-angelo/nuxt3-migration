      ...
      <v-col class="pt-4 px-0">
        ...
        <section v-else>
          <v-expansion-panels hover tile>
            <v-expansion-panel @change="getCompanyAssociations(item.left_side_id)" v-for="item in associationsList" :key="item.id" class="elevation-0">
              <v-expansion-panel-header disable-icon-rotate class="font-body-large remove-min-height py-0 pl-2 pr-0">
              ...
              <v-expansion-panel-content eager class="pt-2">
                <ul>
                  <li v-for="item in companyList" :key="item.right_side_id" class="subtitle-2 mb-1 company-link">
                    <NuxtLink :to="`/company/${item.right_side_id}`" class="stroke--text">
...
export default {
  ...
  mounted() {
    ...
    this.getGroupAssociations(this.associations)

    this.routeId = this.$route.params.id
  },
  watch: {
    associations(newValue) {
      this.getGroupAssociations(newValue)
    }
  },
  methods: {
    ...
    async submitForm() {
      ...
      if(exists.length) {
        ...
        await this.$store.dispatch('fetchAssociations',`includes=left_side,association_type&right_side_id=${id}`)      
