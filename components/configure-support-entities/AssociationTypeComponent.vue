<template>
  <div>
    <v-row class="mr-2">
      <v-col class="pt-0">
        <h6 class="bordered-title pl-3">Tipos de associação</h6>
      </v-col>
    </v-row>
        ...
        <v-col class="d-flex justify-end align-center">
          ...
          >
            <template v-slot="{ dialog, submitted }">
              <SupportEntitiesAssociationTypeForm
                :isEditing="false"
                :dialog="dialog"
...
export default {
  ...
  methods: {
    ...
    formatData(list) {
      const entityList = list.map(({ id, name, category}) => ({
        id,
        name,
        category: this.getEnum(category, this.meta[this.enum])
      }))
      return entityList
    }
  }
