<template>
  <div>
    <v-row class="mr-2">
      <v-col class="pt-0">
        <h6 class="bordered-title pl-3">Origem de oportunidade</h6>
      </v-col>
    </v-row>
...
export default {
  ...
  methods: {
    ...
    formatData(list) {
      const entityList = list.map((origin) => {
        origin.active = origin.active ? 'Ativo' : 'Inativo'
        return origin
      })
      return entityList
    }
